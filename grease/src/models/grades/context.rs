...
impl GradesContext {
    pub async fn for_members_during_semester(
        ...
        Ok(Self {
            ...
            events: events
                .into_iter()
                .map(|event| (event, gigs.drain_filter(|gig| gig.event == event.id).next()))
                .collect(),
            attendance,
        })
    }
    ...
    pub fn weeks_of_attendance(&self, email: &str) -> impl Iterator<Item = WeekOfAttendances> {
        let start_of_semester = self
            .events
            .first()
            .and_then(|event| event.call_time)
        ...
        let first_sunday = start_of_semester
            - Duration::days(start_of_semester.weekday().number_from_sunday() - 1);

        std::iter::successors(Some(first_sunday), |sunday| {
    ...
    async fn for_members_during_semester(
        ...
        for attendance in attendances {
            let context = all_context
                ...
                .or_default();
            let rsvp_issue = event_types
                .get(&attendance.event)
                .and_then(|event_type| Event::rsvp_issue_for(event_type, attendance, is_active));
